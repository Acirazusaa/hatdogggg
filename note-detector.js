 ...
 function NoteDetector(dataSize, sampleRate, windowType)
 {
    this.conf =
    {
        ...
        track_lone_ms       : 100,   // 100 ms of sustained lone estimate
        track_cons_ms       : 100,   // 100 ms of sustained consensus estimate

        detrack_min_volume  : 0.02,  // require higher minimum volume
        detrack_est_none_ms : 50,    // 50 ms of no estimates
        detrack_est_some_ms : 50,    // 50 ms of some estimates with no consensus

        stable_note_ms      : 100,   // require longer stable note period
    };
    /*{
    ...
    /* {
     ...
     this.startTracking_ = function(hz, start)
     {
         ...
         this.detectors[0].volume_min  /= 1;  // acx
         this.detectors[1].threshold   *= 1;  // yin
         this.detectors[2].peak_ignore /= 1;  // mpm
     }
